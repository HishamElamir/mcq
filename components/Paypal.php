...
class Paypal extends \yii\base\Component {
     ...
     */
    public $paypalUrl = 'https://www.sandbox.paypal.com/webscr&cmd=_express-checkout&useraction=commit&token=';
    ...
    public function SetExpressCheckout($paymentInfo = array()) {
        ...
        $returnURL = 'http://localhost:8012/projects/mcq/web/index.php?r=teacher/' . urlencode($this->returnUrl);
        $cancelURL = 'http://localhost:8012/projects/mcq/web/index.php?r=teacher/' . urlencode($this->cancelUrl);

        $this->apiUsername;
        $this->apiPassword;
        $this->apiSignature;
        $nvpstr = '&PAYMENTREQUEST_0_AMT=' . $amount . '&PAYMENTREQUEST_0_PAYMENTACTION=' . $paymentType . '&CURRENCYCODE=' . $currencyCode . '&RETURNURL=' . $returnURL . '&CANCELURL=' . $cancelURL . '&DESC=' . $desc . '&QTY=' . $quantity;
...
//        exit();
    ...
    public function hash_call($methodName, $nvpStr) {
        ...
        $API_UserName = \yii::$app->Paypal->apiUsername;
        $API_Password = \yii::$app->Paypal->apiPassword;
        $API_Signature =\yii::$app->Paypal->apiSignature;
